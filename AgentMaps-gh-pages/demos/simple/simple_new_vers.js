...
function setup() {
	...
	agentmap.buildingify(bounding_box, map_data, undefined, {"color": "black", "weight": 1.5, "opacity": 1}, units_data, streets_data);

	var buildings = agentmap.units.getLayers()

	var factories = [buildings[0],buildings[1]];
	for (let i = 0; i < 2; i++) {
		factories[i].setStyle({color: "green"});
	}
	var apartments = [buildings[3],buildings[2],buildings[4],buildings[5]];
	for (let i = 0; i < 4; i++) {
		apartments[i].setStyle({color: "blue"});
	}
	var parks = [buildings[6],buildings[7],buildings[8]];
	for (let i = 0; i < 3; i++) {
		parks[i].setStyle({color: "black"});
	}

	//Split the map's units into residential and commercial zones.
	var residential_streets = ["way/549667173"],
	commercial_streets = ["way/549667186"];
	agentmap.zoned_units = getZonedUnits(agentmap, residential_streets, commercial_streets);

	//Use only a subset of the zoned units.
	agentmap.zoned_units.residential = pick_random_n(agentmap.zoned_units.residential, 4),
	agentmap.zoned_units.commercial = pick_random_n(agentmap.zoned_units.commercial, 3);
